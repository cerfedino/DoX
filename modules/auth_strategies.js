...
async function registerUser(req, user, password, done) {
    ...
    let verification_link = `${config.webserver.https_enabled ? 'https:' : 'http:'}//${config.webserver.domain}${config.webserver.https_enabled?"":":"+config.webserver.port}/auth/verify/${new_user._id}/${new_user.token}`
