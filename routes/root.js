...
router.put("/user/changeemail", checkAuthenticated, async (req, res) => {
    ...
    if (req.body.email) {
        ...
        tags.token = tkn;
        let verification_link = `${config.webserver.scheme}${config.webserver.domain}${config.webserver.scheme === "https://"?"":":"+config.webserver.port}/auth/changeemail/${req.user.user_id}/${tkn}`
